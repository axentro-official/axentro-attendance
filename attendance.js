...
// ==== Axentro patch: restore admin employee methods ====
if (typeof AttendanceManager !== 'undefined') {
    if (typeof AttendanceManager.prototype.loadEmployees !== 'function') {
        AttendanceManager.prototype.loadEmployees = async function() {
            try {
                if (typeof db === 'undefined') throw new Error('Database not available');
                const { data: emps, error } = await db.from('employees')
                    .select('code, name, email, is_deleted, created_at, is_first_login, face_enrolled, profile_image_url')
                    .eq('is_deleted', false)
                    .order('created_at', { ascending: true });
                if (error) throw error;
                const employees = emps || [];
                window.employeesList = employees;
                if (window.adminManager && typeof window.adminManager.populateEmployeesTable === 'function') {
                    window.adminManager.employeesList = employees;
                    window.adminManager.populateEmployeesTable(employees);
                    window.adminManager.updateEmployeesCount(employees.length);
                }
                const totalStat = document.getElementById('adminTotalEmp') || document.getElementById('totalEmployeesStat');
                if (totalStat) totalStat.textContent = employees.length;
                return employees;
            } catch (e) {
    ...
    if (typeof AttendanceManager.prototype.setupSearch !== 'function') {
        AttendanceManager.prototype.setupSearch = function() {
            const searchInput = document.getElementById('adminSearchInput') || document.getElementById('searchInput');
            if (!searchInput || searchInput.dataset.bound === 'true') return;
            searchInput.dataset.bound = 'true';
            searchInput.addEventListener('input', (e) => {
                const query = String(e.target.value || '').toLowerCase().trim();
                const all = window.employeesList || [];
                const filtered = !query ? all : all.filter(emp =>
                    String(emp.name || '').toLowerCase().includes(query) ||
                    String(emp.code || '').toLowerCase().includes(query) ||
                    String(emp.email || '').toLowerCase().includes(query)
                );
                if (window.adminManager?.populateEmployeesTable) {
                    window.adminManager.populateEmployeesTable(filtered);
                }
            });
        };
    }

    if (typeof AttendanceManager.prototype.adminDirectAtt !== 'function') {
        AttendanceManager.prototype.adminDirectAtt = async function(type, code, name) {
            if (!window.sessionDescriptor) {
    ...
    if (typeof AttendanceManager.prototype.adminDeleteEmp !== 'function') {
        AttendanceManager.prototype.adminDeleteEmp = async function(code, name) {
            const ok = await (window.ui?.showConfirmation ? window.ui.showConfirmation({
                title: 'تأكيد حذف الموظف',
                message: `سيتم حذف الموظف ${name} من الواجهة وإيقافه. هل تريد المتابعة؟`,
                type: 'danger',
                confirmText: 'حذف',
                cancelText: 'إلغاء'
            }) : Promise.resolve(confirm(`حذف الموظف ${name}؟`)));
            if (!ok) return;
            if (!window.sessionDescriptor) {
    ...
    if (typeof AttendanceManager.prototype.adminResetFace !== 'function') {
        AttendanceManager.prototype.adminResetFace = async function(code, name) {
