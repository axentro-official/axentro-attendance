...
class SupabaseClient {
    ...
    async registerEmployee(employeeData) {
        try {
            ...
            try {
                const { data, error } = await this.client.rpc('create_employee_secure', payload);
                if (error) {
                    rpcError = error;
                } else {
                    createdEmployee = Array.isArray(data) ? data[0] : data;
                }
            } catch (e) {
                rpcError = e;
            }
            ...
            if (!createdEmployee) {
                const details = rpcError?.message || rpcError?.details || '';
                throw new Error(details || 'قاعدة البيانات تحتاج دالة create_employee_secure');
            }

            await this.sendNewEmployeeEmails({
                name: employeeData.name?.trim(),
                email: employeeData.email || null,
                code: createdEmployee.code || payload.p_code,
                password
            });
